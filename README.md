...
- Export targets **single-T0, planar PrusaSlicer G-code**, in text or binary form, for **Prusa/Marlin (`marlin` or `marlin2`) and Klipper**, with full slicer settings and layer/feature annotations. The exact first model extrusion must be preceded by explicit millimetres (`G21`), absolute XYZ (`G90`), relative extrusion (`M83`), a known position/feed rate, and an established retract/restart handoff. Outstanding retraction without a separate restart blocks export; a measured startup restart difference can be reviewed as described below. Model extrusion must remain relative; absolute-extrusion files can be previewed but cannot be exported. Linear moves and ordinary XY I/J or R arcs are interpreted.
...
Input-command interpretation and the generated-command allowlist are separate. Original settings remain inherited: the brim does not replay tool selection/calibration, monitoring, current, temperature, fan, flow, pressure advance or jerk commands. Only the feed and acceleration changes needed by the brim and original continuation are emitted.

- **No issues:** export proceeds through the normal G-code review.
- **Minor concerns:** supported fallback insertion, unknown incoming XY with a complete absolute first travel, and geometry advisories remain visible; export is allowed.
- **Blocking issues:** hard failures disable export. A separate class of unresolved assumptions permits preparing a candidate, but downloading requires accepting each listed assumption and a second confirmation for that exact output. The review shows source lines, commands, assumptions and consequences. No global bypass or remembered consent is stored; changing the file, settings or export mode invalidates the review.

In export review, **Acknowledge and collapse** accepts an individual assumption and removes its expanded card from the pending list. A compact acknowledged summary keeps the details accessible, with an undo action that requires acknowledgement again. Minor concerns can also be acknowledged and collapsed. This frees space for the diff without changing export eligibility: hard blockers remain visible without dismissal, and exports with assumptions still require the separate final risk confirmation. Closing and reopening the review resets acknowledgement.

The [input-command catalog](docs/input-commands.md) lists recognized commands, firmware scope, sources and exclusions. It includes Marlin/Prusa thermal controls, monitoring, current, display and reporting settings, plus Klipper heater/fan, sensor, shaping, current, display and reporting built-ins. These remain inherited without generating extra setup/restore commands. Exact command names are matched; firmware-specific additions use the slicer flavor, with no printer-model exceptions. Recognized settings remain insertion boundaries, and dedicated mode/acceleration checks still apply.

Opaque startup cleaning/parking/calibration commands (`G12`, `G27`, `G30`, `G425`, `G427`) are treated as barriers: position, feed, units and positioning/extrusion modes must be explicitly established afterward. Their hidden effects on retraction and deposited material require a reviewable assumption. Other unverified startup M commands require an assumption that they preserve the established interpretation and geometry. They invalidate known acceleration, which is inherited until explicitly set again. This logic does not match printer names or execute embedded startup templates.

Retraction accounting uses the actual command sequence. If an explicit retract during startup or first-layer preparation is followed by a separate stationary restart that leaves a difference, the app reports its measured amount and preserves the original handoff balance. Accepting that concern means accepting that annotated positive model extrusions deposit material; these paths remain in the footprint even while the nominal startup balance is being repaid. There is no printer-specific `0.04 mm` exception or silent extra prime. The same handling applies to both insertion strategies and export modes. Missing restarts still block export.

Inches (`G20`, even if later followed by `G21`), missing established units/modes/position, corrupt binary data, invalid geometry/configuration, unsupported transforms or model motion, missing insertion points, and disallowed generated commands cannot be overridden. Inch values are not converted; a preview of blocked inch-based input is not an accurate unit conversion. Standard-mode E-counter dependencies remain hard blockers for that mode. Accepting startup assumptions never expands the output allowlist.
