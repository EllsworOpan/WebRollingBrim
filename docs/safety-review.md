...
## Binary G-code support

The app now accepts version-1 `.bgcode` using supported INI metadata, unencoded/MeatPack G-code, and none/DEFLATE/heatshrink compression. Import verifies declared CRC32 on all blocks, validates structure/order/codecs, bounds decompressed output, and runs the same motion/geometry/export checks on decoded commands. Binary export validates the generated brim with the runtime allowlist before rebuilding only the affected G-code block as bounded uncompressed blocks. Metadata, thumbnails and untouched binary blocks retain their original bytes. The binary diff is explicitly labelled as decoded commands, and its download shares the prepared binary output.

Tests include a freshly sliced original clearance plate, all supported codec combinations, optional CRCs, exact decoded-command preservation, unchanged binary-byte preservation, a separate decoder reading exported files, corruption/truncation/version/order rejection, decompression limits, and prohibited-command rejection at binary export. Binary metadata comments in the review are reconstructed for display; no claim is made that the entire exported binary file remains byte-identical. These checks do not establish physical printer behavior.

Verification after this addition: 181 tests pass with one optional private-file test skipped; TypeScript, license checks and the production build pass. Browser checks load the real binary sample, render its 3D preview, and show the decoded insertion review with the binary output name. The browser reports download started; binary contents and round trips are verified by the export tests.


## State-based startup review and scoped overrides

Startup settings and generated commands have separate policies. Recognized monitoring/current settings remain inherited. Opaque numeric cleaning/calibration routines invalidate tracked position, feed and modes until explicit source commands recover them. Their hidden retraction/deposition effects, and otherwise unverified startup M commands, produce structured concerns with source lines, assumptions and consequences. Unknown acceleration remains inherited until an explicit setting restores knowledge.

The UI distinguishes no issues, minor concerns and blocking issues. Only structured concerns are overridable: the user reviews the concrete generated output, accepts each assumption and confirms the risks in a second dialog. Consent lives only in the current review, and download checks it again. File, setting or export-mode changes close the review. Text and binary export APIs reject unaccepted concerns by default; candidate preparation explicitly supplies the listed IDs and retains all concerns for download confirmation. Hard blockers and the late generated-command allowlist remain mandatory.

A startup retract followed by an explicit stationary restart can leave a known difference. Detection depends on commands and amounts, with no printer/model/template matching or fixed tolerance for a particular profile. The candidate preserves that balance in both earlier-travel and model-start handoffs. Its review explicitly identifies the assumption that original annotated model extrusion deposits material; these paths are retained while the nominal startup balance is repaid. Missing restarts still block export. No source commands are rewritten and no extra compensation is added to the original continuation.

Regression coverage includes unrelated retract/restart lengths, both insertion strategies and export modes, independent retraction replay, hard-blocker precedence, acceptance at every assembly sink, binary round trips, and late prohibited-command injection despite acceptance. A private binary fixture can be included via `ROLLING_BRIM_TEST_BGCODE`; it is not bundled with the site. These checks do not establish physical printer behavior.

Verification for this update: 205 tests passed with the supplied private binary fixture enabled; one unrelated optional text fixture test was skipped. TypeScript, license checks and the production build passed. Browser checks verified binary review, individual concern acceptance, the separate download confirmation, download-start reporting, cancellation back to review, cleared acceptance on a fresh review, and non-overridable inch-mode rejection. Binary output preservation was independently decoded in the regression suite. The review layout keeps its diff visible while the concern list scrolls.
