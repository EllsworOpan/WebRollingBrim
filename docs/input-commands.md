...
The catalog contains 131 exact command names: the existing 47 numeric compatibility entries, 52 Marlin/Prusa entries, and 32 Klipper built-ins. This adds 76 names to the previous catalog. Motion, homing, coordinate resets, unsupported transforms and opaque startup routines also have dedicated handling outside this catalog.
...
| Motor enabling, current, current reports/storage, driver thresholds/timing | `M17`, `M906`, `M907`, `M908`, `M909`, `M910`, `M911`, `M912`, `M913`, `M914`, `M919`, `M920` |
| Filament monitoring, homing speed, software endstops, filament-change parameters | `M407`, `M412`, `M591`, `M210`, `M211`, `M603` |

`M149` selects **temperature** units; it does not establish millimetres for motion. `G21` remains required, and any `G20` still blocks export. `M210` changes homing feed, not the active `G1 F` feed. Configuring a filament-change length with `M603` does not execute a filament change.

The additions follow the [Marlin command reference](https://marlinfw.org/meta/gcode/) and [Prusa Buddy command reference](https://help.prusa3d.com/article/buddy-firmware-specific-g-code-commands_633112). The latter documents Prusa-specific cooling, lighting and filament monitoring behavior. Driver-current storage via `M910` concerns the DAC's current values, not a snapshot of the brim's acceleration or E position; see [Marlin M910](https://marlinfw.org/docs/gcode/M910.html).
